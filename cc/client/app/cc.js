...
const _requireEntityClass = type => () => require(`./entity/${type}`).default;
...
export const ModelType = {
	...
	"pharmacokinetic": {
		...
		beta: false,
		active: {
			// DISABLED: Not used in production
			// "research": true, 
			"learning": false,
			"teaching": false
		},
...
export const Socials = {
	"google": {
		"title": "Google",
		"color": "#dd4b39",
		"api": `${import.meta.env.VITE_CC_URL_CCAPP}/auth/google-redirect?domain=${Application.domain}`,
	},
	"facebook": {
		"title": "Facebook",
		"color": "#3B5998",
		"api": `${import.meta.env.VITE_CC_URL_CCAPP}/auth/facebook-redirect?domain=${Application.domain}`,
	},
	"linkedin": {
		"title": "LinkedIn",
		"color": "#007bb5",
		"api": `${import.meta.env.VITE_CC_URL_CCAPP}/auth/linkedin-redirect?domain=${Application.domain}`,
	}
}

/**
 * Moved from the models view to facilitate general application management
 */
const getModelTypes = (modelFilter) => {
  const ModelTypes = ImmutableJS.Seq(ModelType)
    .filter((v) => !v.beta)
    .map((v, k) => ({ ...v, type: k }));
  ...
  if (modelFilter) {
    ...
    values = ImmutableJS.Seq([modelType.type]);
  }

  return values;
};

const cc   = { _, config: AppConfig, constants, store: storage,
  socket, request, getenv, ImmutableJS, getModelTypes
};
window.cc  = cc;
