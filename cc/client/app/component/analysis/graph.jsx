...
export default class Graph extends React.Component {
	update({data, x, y, all, components}) {
		...
		this.self.dimension(dim)
			...
			.compose(y.map((v) => DC.scatterPlot(this.self).symbolSize(3).highlightedSize(5).group(dim.group(), components[v.id].name).colors(colors[i % n]).valueAccessor(((i, e) => e.key[i]).bind(null, ++i))).toArray())
			.render();
	}
	...
	componentDidMount() {
		this.self = DC.compositeChart(ReactDom.findDOMNode(this))
