...
export default function BulkPolyRNA({ optionsLayout, optionsMethod, actions, state, editable }) {

	const bulkRNAPath = ['contextSpecificiMAT', 'typeBulkPolyaRNA'];
	const componentStates = state.getIn(bulkRNAPath).toObject();
	const normalizationMethodValue = componentStates.method;
	const minimumActivityLevelValue = componentStates.methodActivity;
	const methodActivityRange = componentStates.methodActivityRange;
	...
	const handleSelectMethod = (e) => {
		...
		switch (e.id) {
			...
			case 'cpm': optRange.min = 0; optRange.max = disabledRangeValue; optValue = NaN; break;
			default: ;
		}
		...
		<div className="bottom">
			...
			<div style={styles.disabledStyle}>
				...
				<dt>
					<dd>Method:</dd>
					<Options none={'Select'}
						value={normalizationMethodValue}
						get={getSelectedOption}
				...
				<dt>
					{!(methodActivityRange.max === disabledRangeValue || Number.isNaN(stateData.methodActivity)) ? (
						<SliderInput
							...
							value={minimumActivityLevelValue}  
							maxLength={3}
