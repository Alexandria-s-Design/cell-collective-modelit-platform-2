...
const isDraggingMetabolite = (dragging) => {
	if (!dragging) return;
	if ( dragging instanceof Metabolite ) {
		return true;
	} else if (dragging.className === 'Metabolite') {
		return true;
	}
}
...
const rDroppable = ({selected: {Reaction: reaction}, model, dragging, actions}, r, l) => {
	const addMetabolite = (r) => {
		...
		const updates	= [ ];
		const key	= `-${Math.abs(reaction.id)}${new Date().getTime()}`;
	...
	return (
		<div className={r ? "reactant" : "product"}>
			<Droppable onDrop={() => {
				if (isDraggingMetabolite(dragging)) {
					addMetabolite(r);
				}
			}}>
				{l}
...
const rAdd = (props, r, p) => {
	...
	{r && rDroppable(props, true, <FormattedMessage id="Regulation.DropReactantMetabolite.DropReactantText" defaultMessage="Drop Reactant" />
	)}
	{p && rDroppable(props, false, <FormattedMessage id="Regulation.DropProductMetabolite.DropProductText" defaultMessage="Drop Product" />
	)}
</div>
}
	...
	render ( ) {
		...
		return (
			<span>
				...
				<div>
					<Panel {...view} title={title(true, false, "Reactants")} width="50%" height="90%" minWidth={80} >
						<ElementsView expandedView={expandedView} {...props} isReactants={true}/>
					</Panel>
					<Panel {...view} title={title(false, true, "Products")} width="50%" height="90%" minWidth={80} left="50%" className="right">
...
const Header = props => {
	...
	return isDraggingMetabolite(dragging) && e && parentWidth < 2 * minWidth ? rAdd(props, true, true) : title(e, name);
};
