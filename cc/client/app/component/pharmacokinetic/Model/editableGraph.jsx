...
export default (readonly, styles, selection, onSelect, onUpdate, nodeColor, edgeStyle = Graph.edgeStyle, interact = {}, state = {}) => {
	...
	class self extends React.Component {
		...
		componentDidUpdate(_, prevState) {
			this.setViewport(this.refs.master.self.getViewport());
		...
		showEditing() {
			let comp;
			if (this.state.editing && this.props.selected && (comp = this.props.selected.PKCompartment)) {
				const { editedText, master } = this.refs;
				...
				c = master.transformInv(norm);
				const tp = master.self.getTextPosition({ x: norm.x, y: norm.y, label: comp.name });
		...
		onChangeViewport(e) {
			...
			selection && f(this.refs.selection.self);
			this.dragging && f(this.draggingRef.current.self);
			this.moving && f(this.refs.moving.self);
		...
		onDragStart(e, f) {
			...
			if (props.editable) {
				...
				let entity = model.PKCompartment[e];

				if (entity === this.state.movableNode) {
					this.moving = { entity: entity };
					this.update(props);
					this.refs.moving.self.setViewport(this.refs.master.self.getViewport());
					!props.selected.Layout && props.actions.batch(LayoutControl.creator(Application.defName(model.Layout, "New Layout "), model.PKCompartment, props.editable));
				} else {
					this.dragging = { entity: entity };
					this.draggingRef.current.self.setViewport(this.refs.master.self.getViewport());
				}
			}
			return true;
		}
		transformCoords(e, d) {
			d = d || this.dimension;
			e = this.refs.master.transform(e);
		...
		onDragStop(_, f) {
			if (this.moving) {
				...
				const e = this.props.selected.Layout.pkcompartments[d.entity.id];
				this.props.actions.batch([new Update(e, "x", d.x), new Update(e, "y", d.y)]);
			} else if (this.dragging) {
		...
		getNodeLayoutCoords(props, e) {
			...
			return l ? (l.pkcompartments[e.id] || e) : e;
		}
		...
		transformNodes(props, e, style, color) {
			...
			return e.map((v, e) => (e = this.getNodeLayoutCoords(props, v), {
				uniqid: v.id, 
				label: v.name, 
				color: c && c(v), 
				entity: v, 
				style: style[isExternal(v)], 
				x: e.x, 
				y: e.y
			}))
			.toObject();
		...
		update(props) {
			...
			if (props.model.id !== undefined) {
				...
				const calcDim = (n, l) => {
					const d = n.length ? NetViz.layout.normalize(Seq(n).map(n => Seq(n).toObject()).toArray(), l) : { minX: 0, minY: 0, maxX: 1, maxY: 1 };
					let ch = false;
				...
				let d;
				d = calcDim(Seq(nodes).map(n => Seq(n).toObject()).toArray()); this.defaultDimension = d.d;
			...
			const graph = this.refs.master.self;
		...
		updateMoving(props) {
			...
			this.refs.moving.self.set(nodes, edges).draw();
		}
		adjustMoving(nodes) {
			const d = this.moving;
			let e = d;
		...
		render() {
			...
			const myOnClick = (e, el) => {
				...
				if (model.PKCompartment) {
					...
					if (el && el.node && interact.canNodeClick && interact.canNodeClick(e, el, this.props) && interact.onNodeClick && interact.onNodeClick(e, this.props, model.PKCompartment[nid], this)) return;
					if (onSelect && (el === null || el.node) && onSelect(this, model.PKCompartment[nid])) return;
					if (el && el.label && interact.canLabelClick && interact.canLabelClick(e, el, this.props) && interact.onLabelClick && interact.onLabelClick(e, this.props, model.PKCompartment[lid], this)) return;
			...
			const myOnDblClick = (e, el) => {
				...
				nid && interact.canNodeDblClick && interact.canNodeDblClick(e, el, this.props) && interact.onNodeDblClick && interact.onNodeDblClick(e, this.props, model.PKCompartment[nid], this);
			...
			return (
				<div className="canvas" onMouseOver={disable && disable.bind(null, true)} onMouseLeave={disable && disable.bind(null, false)}>
					...
					<Graph ref="master" {...p} onLoad={this.onLoad.bind(this)} onChangeViewport={this.onChangeViewport.bind(this)} nodesClickable={(e, el) => (!!selection || !readonly)}
					...
					{!readonly && editable && (<Graph ref="moving" {...p} />)}
					{selection && (<Graph ref="selection" {...p} styles={styles.selection} />)}
					{!readonly && editable && (<Graph ref={this.draggingRef} {...p} styles={styles.dragging} />)}
	
					{this.state.editing && this.props.selected && this.props.selected.PKCompartment && (<Inp type="text" ref="editedText" style={{ left: "0px", top: "0px" }} onKeyUp={inpKeyUp} onBlur={saveEdited} onChange={(self) => { self.setState({ props: { value: self.refs.self.value } }); }} value={this.props.selected.PKCompartment.name} />)}
