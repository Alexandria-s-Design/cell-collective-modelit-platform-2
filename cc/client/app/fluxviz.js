import showMessages from "./message/index";
...
const patch_model = model => {
		...
		for ( let i = 0, a = model.reactions.length ; i < a ; ++i ) {
				...
				model.styles[`reaction-${reaction.id}`] = {
					width: reaction.pixel != undefined ? reaction.pixel : 1,
					arrow: { texture: "" },
					label: { }
				}
				
				model.styles[`reaction-edge-${reaction.id}`] = {
					width: reaction.pixel != undefined ? reaction.pixel : 1,
					arrow: {
						texture: "/assets/images/fluxviz-arrow.png",
					},
					label: {}
				}
		}
...
const path_kinetic_model = model => {
	model.styles = {};
	model.compartments = model.compartments || { };
	const compartment_ids	= Object.keys(model.compartments);
	const compartment_metabolite_count = _objectify(compartment_ids, c => ({ [c]: 0 }));
	const id_metabolite_map 	= _objectify(model.metabolites, metabolite => ({ [metabolite.id]: metabolite }));
	const compartment_reactions		= _objectify(compartment_ids, c => ({
		[c]: _objectify(compartment_ids, c => ({ [c]: [ ] })) 
	}));
	...
	model.reactions.forEach(reaction => {
		...
		// get all compartments from metabolites in reaction.
		[...reaction.reactants, ...reaction.products].forEach(metabolite_id => {
			let speciesId = getChemicalSpeciesId(metabolite_id);
			compartments.push(id_metabolite_map[speciesId].compartment);
		});

		reaction.compartments = Array.from(new Set(compartments));


		compartment_ids.forEach(compartment_id => {
			reaction.compartments.forEach(compartment => {
				if (reaction.compartments.includes(compartment_id)) {
					compartment_reactions[compartment][compartment_id].push(reaction);
					compartment_reactions[compartment_id][compartment].push(reaction);
				}
			});
	...
	fluxviz.logger.warn("Patching Compartments...");
	for (let i = 0; i < compartment_ids.length; ++i) {
			const compartment	= compartment_ids[i];
			const name = model.compartments[compartment] || compartment;
			...
			model.compartments[compartment] = { name: name,
					...
					// metabolite_density: metabolite_density 
				};
...
const get_metabolite_nodes_and_reaction_edges = (metabolites, reactions) => {
		...
		for ( let i = 0, n = metabolites.length ; i < n ; ++i ) {
				...
				const node 	= { label: m.name || m.id, type: type,
						style: m.style ? m.style : "metabolite-compartment-" + m.compartment,
						object: m };
		...
		for ( let i = 0, n = reactions.length ; i < n ; ++i ) {
				...
				const node 	= { label: r.name || r.id, type: type,
		...
		const edges = reactions.map(r => {
						...
						const reactants = r.reactants.filter(r => getChemicalSpeciesId(r) in nodes.metabolites);
						const products  = r.products .filter(p => getChemicalSpeciesId(p) in nodes.metabolites);
						if ( reactants.length && products.length ) {

							let edgeToBePushed;
							let productsInReactions = [];

							if(r.pixel != 0) {

								for ( let i = 0; i < products.length ; ++i ) {
									const product = getChemicalSpeciesId(products[i])
									if ( product in nodes.metabolites ) {
											productsInReactions.push(nodes.metabolites[product]);
									}
								}

								for ( let i = 0; i < reactants.length ; i++ ) {
									const reactant = getChemicalSpeciesId(reactants[i]);

									for ( let j = 0; j < productsInReactions.length ; j++ ) {

										if ( reactant in nodes.metabolites ) {
											edgeToBePushed = {
												type:   type,
												source: nodes.metabolites[reactant],
												target: productsInReactions[j],
												object: r,
												style:  `reaction-edge-${r.id}`
											}
											...
											if ( r.reversible ) {
												edgeToBePushed = {
													type:   type,
													source: productsInReactions[j],
													target: nodes.metabolites[reactant],
													object: r,
													style: `reverible-reaction-${r.id}`
												}
													edges.push(edgeToBePushed);
											}
										}
...
const excludedColors = [ "rgb(0,255,0)", "rgb(255,0,0)", "rgb(0,0,255)"];
...
fluxviz._colors				= { sinks: "rgb(0,255,0)", exchanges: "rgb(255,0,0)", demands: "rgb(0,0,255)" };
fluxviz.color					= (type) => {
		const colors = fluxviz._colors

		if ( !(type in colors) ) {
			colors[type] = generateRandomColor();
		}

		return colors[type];
}

fluxviz.render = async (element, model, updateSelectedCompartment, { styles = { }} = { }, props) => {
		fluxviz.logger.warn("Patching Model...", model);
		if (model.modelType === "kinetic") {
			path_kinetic_model(model);
		} else {
			patch_model(model);
		}
		...
		fluxviz._graph = new ccNetViz(element, {
				...
				styles: Object.assign({ },
						{
							node: {
								texture: "/assets/images/fluxviz-circle.png",
								label: {
									hideSize: 6
								},
		...
		if (compartments && compartments.length) {
			nodes = _objectify(compartments, next => {
					...
					let nodes = null;
					let edges = null;
					if (compartment.subsystem && compartment.subsystems.length ) {
							// parallelize this.
		...
		const renderOneLevelDownNodes = async (node, graph) => {
			...
			if ( node.nodes ) {
				...
				if(model.modelType === "kinetic"){
							...
							n.style === "metabolite-" + node.style
							);
				}


				const canFilterNodes = node.type.name == "compartment"
				if(canFilterNodes) {
					// we are moving from compartments to metabolites
					const parentStyle = node.style
					nodes = nodes.filter((n => n.type.name =="metabolite" &&  n.style.includes(parentStyle)))
				}
				...
				return { graph, styles, compartment: node.label}
			}
		}
		...
		const multiLevel    = (element, graph) => {
			...
			element.addEventListener("click", async e => {
					...
					if ( !['kinetic','metabolic'].includes(model.modelType) && targets.nodes && targets.nodes.length == 1 ) {
							const node = targets.nodes[0].node;
							await renderOneLevelDownNodes(node, graph);

							const compartment = Seq(props.model.Compartment).find(c => c.name === node.label);
							props.actions.onSelect(compartment || "Compartment")
							updateSelectedCompartment(node)
					}
			});

			element.addEventListener("dblclick", async e => {
				fluxviz.logger.info("Current Level (On Mouse Duble-click): " + level);
				fluxviz.logger.info(history);
				const targets = getSubGraphOnEvent(element, graph, e);
				if ( ['kinetic','metabolic'].includes(model.modelType) && targets.nodes && targets.nodes.length == 1 ) {
					const node = targets.nodes[0].node;
			...
			element.addEventListener("contextmenu", async e => {
					...
					if ( level > 0 ) {
							...
							graph.setViewport({ size: 1, x: 0, y: 0 });
							await drawGraph(graph, nodes, edges, "force");
							showMessages("you went from subcompartment to compartment")

							updateSelectedCompartment(null)
					}
			});
		}
		...
		const renderCompartmentBasedGraph = async (node) => {
			return renderOneLevelDownNodes(node, fluxviz._graph)	
		}

		return { graph: fluxviz._graph, styles, nodes, renderCompartmentBasedGraph };
};
