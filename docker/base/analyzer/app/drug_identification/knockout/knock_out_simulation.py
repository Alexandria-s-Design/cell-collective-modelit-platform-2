...
from pathlib import Path
from ...schemas import DrugSolverBody
...
def write_logs(logs: List[str], log_dir: str, logging=False):
    if (logging == True):
...
def _perform_knockout(
    ...
    reference_solution,
    output_dir: str
) -> tuple[str, pd.DataFrame]: # type: ignore
    """
...
def knock_out_simulation(
    ...
    write_logs([f"[OK] - Time elapsed: {end - start} seconds (multi core)"], output_dir, True)
...
# Result
# cols:	Drug, Target, MOA, d_score
#	row:	0, DrugA, Gene1, Inhibitor, 0.85
...
# Initialization
def main(input_vals: DrugSolverBody):

    # Parameters options
    datadir = input_vals.dir_data
    tissue_spec_model_file = input_vals.model_exported
    context = input_vals.context_name
    disease_up_file = os.path.join(datadir, "tmp", input_vals.up_regulated)
    disease_down_file = os.path.join(datadir, "tmp", input_vals.down_regulated)
    raw_drug_filename = os.path.join(datadir, "tmp", input_vals.drug_csv_path)
    test_all = True
    solver = input_vals.solver.lower()
    test_result = input_vals.test_result
    ...
    if os.path.exists(os.path.join(datadir, "tmp", input_vals.up_regulated)):
       logs_checking_folder.append("[OK] - Disease_Up file exists")
    
    if os.path.exists(os.path.join(datadir, "tmp", input_vals.down_regulated)):
       logs_checking_folder.append("[OK] - Disease_Down file exists")

    if os.path.exists(os.path.join(datadir, "tmp", input_vals.drug_csv_path)):
       logs_checking_folder.append("[OK] - Drug list exists")
    else:
       raise ValueError("[ERROR] - Drug list does not exists")
    ...
    write_logs([ f"[OK] - fluxsolution {fluxsolution}"], output_dir, True)
    ...
    write_logs([
       "[OK] - Gene D score mapped to repurposing drugs saved to:\n {}".format(drug_score_file)
		], output_dir, True)
