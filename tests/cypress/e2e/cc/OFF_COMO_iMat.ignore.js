...
context('iMat',() => {
	...
	it('Save the Model after adding Taxon ID', () => {
	...
	it('Lower and Upper Gene Activity Threshold', () => {
		...
		cy.get('div.editable.enabled').eq(4)  
			...
			.then((text) => {
			const value = parseFloat(text.trim());  
		cy.log('Default Upper Gene Activity Threshold: ' + value)         
			expect(value).to.equal(0) 
		})

		cy.get('.sliderInput .track div').first().then($slider => {
			const sliderWidth = $slider.width();
			cy.wrap($slider).realClick({ x: -4, y: 10 });
		});
		...
		cy.get('div.editable.enabled').eq(3)  
			...
			.then((text) => {
				const value = parseFloat(text.trim())  
				cy.log('Minimum Lower Gene Activity Threshold: ' + value)         
			...
			cy.get('div.editable.enabled').eq(4)  
				...
				.then((text) => {
				const value = parseFloat(text.trim())  
				cy.log('Upper Gene Activity Threshold: ' + value)         
				expect(value).to.equal(0)
		 	})
		
		cy.get('.sliderInput .track div').first().then($slider => {
				const sliderWidth = $slider.width()
				cy.wrap($slider).realClick({ x: 68, y: 10 });
			});
		...
		cy.get('.sliderInput .track div').eq(2).then($slider => {
			const sliderWidth = $slider.width()
			cy.wrap($slider).realClick({ x: -4, y: 10 });
			});
		...
		cy.get('.sliderInput .track div').eq(1).then($slider => {
			const sliderWidth = $slider.width()
			cy.wrap($slider).realClick({ x: -4, y: 10 });
			});	
		cy.get('.sliderInput .track div').eq(2).then($slider => {
		...
		// cy.get('.sliderInput .track div div')
		...
		// 	})
		// })
				//cy.get('.sliderInput .track div div').first().invoke('attr', 'style', 'left: 0%;')
		// cy.get('.track').first() //.click()
		// 	.realMouseDown()
		// 	.realMouseMove(100000, 0, {position: ''})
		// 	.realMouseUp()
  	// 	// .then($slider => {
    // 	// $slider[0].style.left = '0%';
		// 	// $slider.trigger('change');
    // 	// })
		// cy.get('.sliderInput .track div div').eq(1)
  	// 	.then($slider => {
    // 	$slider[0].style.left = '0%';
		// 	$slider.trigger('change');
    // 	})
	...
	it('Bulk Total RNA Sequencing', () => {
		...
		cy.get('div.editable.enabled').eq(8)  
			.invoke('text')              
			.then((text) => {
				const value = parseFloat(text.trim());  
				cy.log('Default Lower Gene Activity Threshold: ' + value)         
		...
		cy.get('div.editable.enabled').eq(9)  
			.invoke('text')              
			.then((text) => {
				const value = parseFloat(text.trim());  
				cy.log('Default Lower Gene Activity Threshold: ' + value)         
		...
		// })
		// cy.get('.sliderInput .track div').eq(4).then($slider => {
		// 	const sliderWidth = $slider.width()
		// 	cy.wrap($slider).realClick({ x: -4, y: 10 });
		// 	});	
		// cy.get('div.editable.enabled').eq(5)  
	...
	it('Single Cell RNA Sequencing (Pre Normalized)', () => {
		cy.contains('Single Cell RNA Sequencing (Pre Normalized)')
			.should('exist')
		cy.get('span[class="checkbox"]').eq(0).click()	
	...
	cy.get('div.editable.enabled').eq(8)  
		...
		//import - need higher version of React (npm install --save-dev cypress-file-upload)
		//has to be imported cypress-file-upload: import 'cypress-file-upload';
		cy.get('input[name="single_cell_rna_upload"]')//.attachFile('Total_RNA_sequencing.csv')
		cy.get('span[class="icon base-menu dropdown-icon-open"]').eq(0).click()
	})

	it('Proteomics (Pre Normalized)', () => {
		cy.contains('Proteomics (Pre Normalized)')
			.should('exist')	
		cy.get('span[class="checkbox"]').eq(0).click()
	...
	it('Boundary Reactions', () => {
		cy.get('input[class="icon base-add"]').eq(0).click()
			.then( () => {
				cy.contains("rA").should('be.visible')
				cy.contains("None").should('be.visible')
				cy.contains("Default").should('be.visible')
				cy.contains("-1").should('be.visible')
				cy.contains("1").should('be.visible')
			})
		cy.get('input[class="icon base-remove"]').eq(0).click()
			.then( () => {
				cy.contains("rA").should('not.exist')
			})	
		cy.get('input[name="boundary_reactions_upload"]')//.attachFile('Total_RNA_sequencing.csv')
	})

	it('Core/Force Reactions', () => {
		cy.get('input[class="icon base-add"]').eq(1).click()
			.then( () => {
			cy.contains("rA").should('be.visible')
		})
		cy.get('input[class="icon base-remove"]').eq(1).click()
			.then( () => {
			cy.contains("rA").should('not.exist')
		})
		cy.get('input[name="core_reactions_upload"]')//.attachFile('Total_RNA_sequencing.csv')		
	})

	it('Exclude Reactions', () => {
		cy.get('input[class="icon base-add"]').eq(2).click()
		.then( () => {
		cy.contains("rA").should('be.visible')
	})
	cy.get('input[class="icon base-remove"]').eq(2).click()
		.then( () => {
		cy.contains("rA").should('not.exist')
	})
	cy.get('input[name="exclude_reactions_upload"]')//.attachFile('Total_RNA_sequencing.csv')	
	})
